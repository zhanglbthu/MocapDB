    ...
    for side in sides:
        ...
        for i, seq_name in enumerate(seq_names):
            ...
            for device_name in device_names:
                ...
                df_list_fps = [data_timestamp_alignment(df) for df in df_list]
        
                min_frames = min(len(df) for df in df_list_fps)
                df_list_fps = [df.iloc[:min_frames].reset_index(drop=True) for df in df_list_fps]
                ...
                for df, key in zip(df_list_fps, keys_to_align):
                ...
                #     pass
                #     '''
                #     '读取包含acc_headsetL, gyro_headsetL, quaternion_leftL的.csv文件'
                #     1. 检查acc_gyro的总帧数是否一致
                #     2. 这两个文件的数据为10条数据共享一个时间戳，现在根据前后两包的时间戳，插值出每条数据的时间戳
                #     3. 将插值后的时间戳和原始数据保存为新的.csv文件
                #     '''
                #     acc_file = find_csv_by_keywords(device_dir, ["acc", "headsetL"])
                #     gyro_file = find_csv_by_keywords(device_dir, ["gyro", "headsetL"])
                #     quat_file = find_csv_by_keywords(device_dir, ["quaternion", "left"])
                    
                #     acc_df = pd.read_csv(acc_file)
                #     gyro_df = pd.read_csv(gyro_file)
                #     quat_df = pd.read_csv(quat_file)
                    
                #     assert len(acc_df) == len(gyro_df), f"acc and gyro frame count mismatch: {len(acc_df)} vs {len(gyro_df)}"
                #     acc_time_col = 'AccTime'
                #     gyro_time_col = 'AccTime'
                #     quat_time_col = 'time'
                    
                #     assert np.all(np.diff(pd.to_numeric(acc_df[acc_time_col], errors='coerce')) >= 0), "ACC timestamps not non-decreasing"
                    
                #     # 时间戳插值
                #     acc_df_interp = interpolate_packet_timestamps(acc_df, acc_time_col)
                #     gyro_df_interp = interpolate_packet_timestamps(gyro_df, gyro_time_col)
                    
                #     # TODO: 获取最短的时间范围，裁剪数据
                #     min_time = max(acc_df_interp[acc_time_col].min(), gyro_df_interp[gyro_time_col].min(), quat_df[quat_time_col].min())
                #     max_time = min(acc_df_interp[acc_time_col].max(), gyro_df_interp[gyro_time_col].max(), quat_df[quat_time_col].max())
                #     acc_df_interp = acc_df_interp[(acc_df_interp[acc_time_col] >= min_time) & (acc_df_interp[acc_time_col] <= max_time)].reset_index(drop=True)
                #     gyro_df_interp = gyro_df_interp[(gyro_df_interp[gyro_time_col] >= min_time) & (gyro_df_interp[gyro_time_col] <= max_time)].reset_index(drop=True)
                #     quat_df = quat_df[(quat_df[quat_time_col] >= min_time) & (quat_df[quat_time_col] <= max_time)].reset_index(drop=True)
                    
                #     # 采样至100fps
                #     acc_df_fps = data_timestamp_alignment(acc_df_interp)
                #     gyro_df_fps = data_timestamp_alignment(gyro_df_interp)
                #     quat_df_fps = data_timestamp_alignment(quat_df)
                    
                #     assert len(acc_df_fps) == len(gyro_df_fps) == len(quat_df_fps), f"Frame count mismatch after resampling: {len(acc_df_fps)} vs {len(gyro_df_fps)} vs {len(quat_df_fps)}"
                    
                #     save_device_dir = os.path.join(save_dir, seq_name, device_name)
                #     os.makedirs(save_device_dir, exist_ok=True)
                    
                #     acc_save_path = os.path.join(save_device_dir, os.path.basename(acc_file))
                #     gyro_save_path = os.path.join(save_device_dir, os.path.basename(gyro_file))
                #     quat_save_path = os.path.join(save_device_dir, os.path.basename(quat_file))
                    
                #     acc_df_fps.to_csv(acc_save_path, index=False)
                #     gyro_df_fps.to_csv(gyro_save_path, index=False)
                #     quat_df_fps.to_csv(quat_save_path, index=False)
